...
class OpenAIService {
  ...
  async summarize(
    ...
    const requiresChunking = needsChunking(
      cleanedText,
      this.config.model,
  ...
  async generateFlashcards(
    ...
    if (!cleanedText || cleanedText.trim().length < 50) {
      throw new OpenAIServiceError(
        'Document content is too short to generate flashcards',
        'CONTENT_TOO_SHORT',
        400
      );
    }
...
- Output MUST be valid JSON.`
  ...
  private async summarizeSingle(
    ...
    return {
      ...
      type,
      tokenUsage,
      cost: calculateCost(tokenUsage, this.config.model),
      chunksProcessed: 1,
      model: this.config.model,
    };
  }
  ...
  private async summarizeWithChunking(
    ...
    const mergeCompletion = await this.callWithRetry(() =>
      this.client!.chat.completions.create({
        model: this.config.model,
    ...
    const finalSummary =
      mergeCompletion.choices[0]?.message?.content?.trim() ?? '';

    if (!finalSummary) {
      throw new OpenAIServiceError(
        'OpenAI returned an empty response during merge',
        'EMPTY_MERGE_RESPONSE',
        502
      );
    }
    ...
    return {
      ...
      cost: calculateCost(aggregatedUsage, this.config.model),
  ...
  private async callWithRetry<T>(
    ...
    try {
      ...
      if (isRetryable && attempt < this.config.maxRetries) {
        ...
        console.warn(
          `[OpenAI] Retrying (attempt ${attempt + 1}/${this.config.maxRetries}) after ${Math.round(delayMs)}ms — error: ${err?.message}`
        );
  ...
  private handleOpenAIError(err: any): never {
    ...
    if (status === 400) {
      throw new OpenAIServiceError(
        `Invalid request to OpenAI: ${err?.message}`,
        code,
        400
      );
    }
    ...
    throw new OpenAIServiceError(
      `OpenAI service error: ${err?.message ?? 'Unknown error'}`,
      code,
