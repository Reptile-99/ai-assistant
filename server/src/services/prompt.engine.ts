...
export const SUMMARY_TYPE_DEFINITIONS: SummaryTypeInfo[] = [
  {
    id: 'short',
    label: 'Short Summary',
...
export function isValidSummaryType(type: string): type is SummaryType {
...
export function buildMergePrompt(
  ...
  const typeInstructions: Record<SummaryType, string> = {
    short:
      'Create a single, cohesive 3–5 sentence executive summary that captures the most important points from all sections.',
    detailed:
      'Synthesize all section summaries into one comprehensive, well-structured summary with clear paragraphs and transitions.',
    bullet:
      'Consolidate all bullet points, removing duplicates and organizing them logically under relevant sub-headings.',
    key_concepts:
      'Merge all key concepts, removing duplicates and organizing them by theme or importance. Present each concept with its definition.',
  };
...
function buildShortSummaryPrompt(text: string, title: string): PromptMessage[] {
  return [
    {
      role: 'system',
      content: `${BASE_SYSTEM_PERSONA}
...
function buildDetailedSummaryPrompt(text: string, title: string): PromptMessage[] {
  return [
    {
      role: 'system',
      content: `${BASE_SYSTEM_PERSONA}
...
function buildBulletPointsPrompt(text: string, title: string): PromptMessage[] {
  return [
    {
      role: 'system',
      content: `${BASE_SYSTEM_PERSONA}
...
function buildKeyConceptsPrompt(text: string, title: string): PromptMessage[] {
  return [
    {
      role: 'system',
      content: `${BASE_SYSTEM_PERSONA}
